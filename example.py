...
class SolarScraper:
    ...
    async def initialize_browser(self, business_type, business_location):
        ...
        try:
            ...
            await self.page.goto(self.ensure_str(f"{self.original_url}&start={self.page_number * 10}"), timeout = 600000)
            print("initialize_browser", self.ensure_str(f"{self.original_url}&start={self.page_number * 10}"))
    ...
    async def scrape(self, business_type, business_location):
        try:
            ...
            while True:
                ...
                try:
                    next_button = await self.page.wait_for_selector('button.pagination-button__09f24__kbFYf.y-css-1ewzev', timeout=12000)  # 12 seconds as an example
                except TimeoutError:
                    print("No more pages.")
                    break
                ...
                next_page_url = f"{self.original_url}&start={self.page_number * 10}"
